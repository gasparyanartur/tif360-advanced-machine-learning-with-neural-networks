...
class TGameBoard:
    ...
    def __init__(self, N_row, N_col, tile_size, max_tile_count, agent, stochastic_prob):
        ...
        random.seed(12345)
        self.tile_sequence = [
            random.randint(0, len(self.tiles)-1) for x in range(self.max_tile_count)
        ]
    ...
    def fn_drop(self):
        ...
        for xLoop in range(len(curtile)):
            if self.tile_y+curtile[xLoop][1] > self.N_row:
                ...
                self.board[self.tile_y+curtile[xLoop][0]:self.tile_y +
                           curtile[xLoop][1], (xLoop+self.tile_x) % self.N_col] = 1
